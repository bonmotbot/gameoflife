class Board {
  ...
  toggleAlive(row, col) {
    const cellIndex = row * this.width + col;
    return this.current[cellIndex] = 1 - this.current[cellIndex];
  }
  ...
  step() {
    for (let r0 = 0; r0 < this.height; r0++) {
      ...
      for (let c0 = 0; c0 < this.width; c0++) {
        ...
        } else if (this.isAlive(r1, c1) && neighborCount == 2) {
          this.buffer[r1 * this.width + c1] = 1;
        } else {
